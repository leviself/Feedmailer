...
class gnuPG
{
	...
	*/
	function Import($KeyBlock)
	{
		...
		// execute the GPG command
		if ( $this->_fork_process($this->program_path . ' --homedir ' . $this->home_directory .
				' --no-permission-warning --status-fd 1 --import',
			$KeyBlock, $contents) ) {
			// initialize the array data
			$imported_keys = array();
	...
	*/
	function EncryptSign($KeyID, $Passphrase, $RecipientKeyID, $Text)
	{
		// initialize the output
		$contents = '';
	...
	/**
	 * Encrypt 	(function by Dr Small)
	 * Usage:	$gpg->encrypt($recipient, $data);
	 **/
	function encrypt($recipient, $data){
		$contents = '';
		if ($this->_fork_process($this->program_path.' --homedir '.$this->home_directory.
			' --armor --trust-model always --no-permission-warning --recipient '.$recipient.
		       ' --comment="Feedmailer One-time Key" --encrypt', $data, $contents)){
			return $contents;
		} else {
			return false;
		}
	...
	*/
	function DeleteKey($KeyID, $KeyKind = 'public')
	{
		...
		// execute the GPG command
		if ( $this->_fork_process($this->program_path . ' --homedir ' . $this->home_directory .
				' --no-permission-warning --batch --yes --status-fd 1 ' .
