<?php	// Load the templates class
	...
	define("COUNT_FEEDS", mysql_result(mysql_query("SELECT COUNT(*) FROM `{$mysql->prefix}feeds`"),0));
?>

				<p>
					<table class="headertext">
						<td>Stats</td>
						<td>Edit & Delete Feeds</td>
					</table>
				</p>
				...
				<div id="sidebar_page">
					...
					<div id="boxpadding">
						<br />
						You can edit a feed by clicking on the link in
						the <strong>ID</strong> column.<br />
						You can also edit a user by clicking on the link in
						the <strong>UID</strong> column. <br />To delete a user, tick the box for the
						specified user and click "Delete".<br /><br />
						<form action="admin_delfeeds.php" method="get">
						Show:
						<select name="results">
...
<?php
	if($page == 0){
		...
		$page_output_next = ($results >= COUNT_FEEDS) ? "" : '<a class="links" href="?page='.$bn_next.'&results='.$results.'&orderby='.$orderby.'">Next &#187</a>';
?>
		
						<form action="admin_delfeeds.php" method="post">
						<table cellpadding="2">
							<tbody>
								<tr>
								...
								<?php	// Show All Users
									$feed->showAllFeeds($orderby,$start,$finish);
								?>
...
<?php
		...
		$page_output_next = ($bn_next >= COUNT_FEEDS) ? "" : '<a href="?page='.$bn_next.'&results='.$results.'&orderby='.$orderby.'">Next &#187</a>';
...
?>
						...
						<table cellpadding="2">
							<tbody>
								...
								<?php	// Show All Users
									$feed->showAllFeeds($orderby,$page,$results);
								?>
